...
class ImagenetDatasetHparams(DatasetHparams, SyntheticHparamsMixin):
    ...
    def initialize_object(
        self, batch_size: int, dataloader_hparams: DataLoaderHparams, indices: List[int] = None
    ) -> DataSpec:
            ...
            image_pipeline.extend(
                [
                    ...
                    ffcv.transforms.NormalizeImage(
                        np.array(IMAGENET_CHANNEL_MEAN), np.array(IMAGENET_CHANNEL_STD), dtype
                    ),
                ]
            )
...
class StreamingImageNet1kHparams(DatasetHparams):
    ...
    remote: str = hp.optional(
        "Remote directory (S3 or local filesystem) where dataset is stored",
        default="s3://mosaicml-internal-dataset-imagenet1k/mds/1/",
    )
    local: str = hp.optional(
        "Local filesystem directory where dataset is cached during operation", default="/tmp/mds-cache/mds-imagenet1k/"
    )
