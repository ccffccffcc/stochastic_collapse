...
class FinetuneExperiment(Experiment):
    ...
    exp_hash: str = hp.required("Experiment hash.")
    load_batch: int = hp.required("Batch size for loading model.")
    ...
    def run(self) -> None:
        ...
        load_state = torch.load(
            "exps/" + self.exp_hash + "/replicate_%d/checkpoints/state_%dba.pt" % (self.replicate, self.load_batch),
            map_location="cpu",
        )
        model.load_state_dict(load_state["state"]["model"])
